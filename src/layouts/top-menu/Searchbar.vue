      ...
      <div class="search-result__content">
        <div class="intro-y col-span-12 sm:col-span-12 md:col-span-12 xxl:col-span-12" v-if="this.search_results.cats.length === 0 && this.search_results.posts.length === 0 && this.search_results.users.length === 0">
          <div class="relative flex items-center">
            <div class="p-1">
              <div class="text-3xl">{{ $t('topbar.search_modal_no_result') }}</div>
              <div class="text-gray-600 mt-2">
                {{ $t('topbar.select_other_query') }}
              </div>
            </div>
          </div>
        </div>
        <div class="search-result__content__title" v-if="this.search_results.cats.length > 0">{{ $t('topbar.filter_categories') }}</div>
        <div class="mb-5">
          <a
        ...
        <div class="search-result__content__title" v-if="this.search_results.posts.length > 0">{{ $t('topbar.filter_posts') }}</div>
        <div class="mb-5">
          <a
        ...
        <div class="search-result__content__title" v-if="this.search_results.users.length > 0">{{ $t('topbar.filter_authors') }}</div>
        <div class="mb-5">
          <a
